...
export HUMAN_EVAL_EXPERIMENT_ENQUEUE_LIMIT
...
HUMAN_EVAL_MODEL_TIMELINE_CREATED_BY="${HUMAN_EVAL_MODEL_TIMELINE_CREATED_BY:-run_daily_pipeline.sh:human-eval:model-timeline}"
...
TRANSLATION_PUBLICATION_API_MODE="${TRANSLATION_PUBLICATION_API_MODE:-responses}"
TRANSLATION_PUBLICATION_REASONING_EFFORT="${TRANSLATION_PUBLICATION_REASONING_EFFORT:-medium}"
...
export TRANSLATION_RESPONSES_RUN_LIMIT
...
# Step 7a0a0: Generate entry-level translation quality histograms
echo "Step 7a0a0: Generating translation quality distributions..." | tee -a "$LOGFILE"
uv run generate_translation_quality_histograms.py \
    --input-csv reference_site/statistics/prompt_evaluation_rows.csv \
    2>&1 | tee -a "$LOGFILE"
