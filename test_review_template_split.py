...
class ReviewTemplateSplitTests(unittest.TestCase):
    def test_translation_page_keeps_translation_tools_and_hides_entity_forms(self):
        ...
        self.assertIn("Current Meineke guidance coverage", template)
        self.assertIn("Guidance coverage diagnostic", template)
        self.assertIn("Missing prompt-rule checks", template)
