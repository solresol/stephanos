...
The reviewed house-style prompt (v2) scores 18.55 percentage points above the minimal prompt (v1); a longer prompt with entry-specific formula guidance (v3) adds a smaller gain and produces many more near-identical outputs. Moving from v1 to v2 is the largest readily available improvement and is equivalent to about 55 months of model progress under the fitted v2 trend. Similarity also rises with model release date, fastest under the detailed prompts, though individual releases sometimes score below their predecessors. For projects that provide guided prompts, component-metric projections reach the provisional 90% reference-similarity marker---used here as an operational definition of human-like output, not human parity---between October 2028 and May 2033.
...
The primary composite is the unweighted arithmetic mean of these four entry-level metric scores. COMET-22 and BLEURT-20 are analysed separately and are not included in the composite.
...
The historical projection solves the fitted line for a score of 0.90. We call this human-like reference similarity: close agreement with the single approved translation, not a calibrated claim of human parity.
...
## 4.2 Development items do not show an extra v2 gain
...
## 4.3 Similarity rises with model release date

The composite slope is positive under every prompt (Table 3).
...
Release date is unrelated to either learned metric under v1. The COMET slope is -0.0018 per year ($R^2=0.058$, $p=0.452$), and the BLEURT slope is -0.0022 ($R^2=0.029$, $p=0.594$). Under v2 and v3, however, both slopes are positive with $p<0.001$ (Table 4). A naive interpretation would be that newer models improve when the prompt supplies the project's conventions, but do not converge on those conventions from the minimal request alone.
...
## 4.6 Exact and near-exact outputs
...
Broadly speaking, the component projections suggest that guided systems may reach the paper's 90% human-like reference-similarity marker between October 2028 and May 2033.
